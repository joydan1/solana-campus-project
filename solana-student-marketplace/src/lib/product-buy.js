...
export default function Marketplace() {
  ...
  const handleBuy = async (item) => {
    ...
    // Confirm purchase
    if (!window.confirm(`Buy "${item.title}" for ${item.price} SOL?`)) return;
            ...
            >
              ...
              <div className="p-4">
                ...
                {item.sold ? (
                  <button
                    disabled
                    className="w-full mt-4 py-2 rounded font-semibold text-black bg-gray-500 cursor-not-allowed"
                  >
                     Sold
                  </button>
                ) : (
                  <button
                    ...
                    className="w-full mt-4 py-2 rounded font-semibold text-black bg-gradient-to-r from-[#00FFA3] via-[#DC1FFF] to-[#9945FF] hover:opacity-90 transition"
                  >
                     Buy Now
                  </button>
                )}
